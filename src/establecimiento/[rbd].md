...
const regionSeleccionada = (() =>{
  ...
  return view(Inputs.select(options, {
    format: (d) =>
      ...
      label:"Región",
          value: options.find(d => d.region == establecimientoSeleccionado["COD_REG_RBD"])
  }));
})()
```
...
const comunaSeleccionada2 = (() => {
  ...
  return view(Inputs.select(options, {
    ...
    label:"Comuna",
    value: options.find(d => d.comuna == establecimientoSeleccionado["NOM_COM_RBD"])
  }));
})()
```

```js
const establecimientoSeleccionado2 = (() =>{
  const options = _.chain([...establecimientos])
...
```js
if(establecimientoSeleccionado2.RBD !== +observable.params.rbd) {
window.location.href = `./${establecimientoSeleccionado2.RBD}`;
...
```sql id=[establecimientoSeleccionado]
SELECT * 
FROM establecimientos
WHERE RBD = ${observable.params.rbd}
```
